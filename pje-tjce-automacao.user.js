...
// @version      1.1.3
// @description  Detecta tipo do select por opções (Meio x Comunicação), estabiliza 'Selecione' com fallback correto, reduz spam de toast e adiciona Copiar ID com ícone ao lado do link.
...
(function () {
  ...
  const CONFIG = {
    ...
    DEFAULT_COMUNICACAO_PREFER: ["Intimação", "Notificação"],
    DEFAULT_MEIO_PREFER: ["Diário Eletrônico"],

    STABILIZER_STABLE_SIG_LIMIT: 4,

    COPY_ID_TOAST_ON_SUCCESS: false, // 👈 deixa discreto
  };
  ...
  let automationsEnabled = !CONFIG.START_AFTER_LOAD;
...
.select2-dropdown{ display:none !important; }
  ...
  const ModAdvogados = (() => {
    ...
    async function clickLawyersSequential(poloName, afterReason) {
      ...
      try {
        ...
        scheduleFinalCheck(afterReason || `Advogados (${poloName})`);
      } finally {
        running = false;
      }
    }
  ...
  /* ------------------------ Copiar ID (ícone ao lado do link) ------------------------ */
  const ModCopiarID = (() => {
    ...
    function processLinks() {
      ...
      // padrão: links de download de documento com texto numérico (ID)
  ...
  function installFinalCheckClickListener() {
    ...
    document.addEventListener("click", (ev) => {
      ...
      if (a.matches('a[data-pje-btn="adv-autor"]')) return scheduleFinalCheck("Advogados Autor");
      if (a.matches('a[data-pje-btn="adv-reu"]')) return scheduleFinalCheck("Advogados Réu");
      if (a.matches('a[title="ADVOGADO"]')) return scheduleFinalCheck("Clique em Advogado (árvore)");
      if (a.closest(".rich-tree") && a.matches("td.rich-tree-node-text a")) return scheduleFinalCheck("Clique em Parte (árvore)");
      if (a.matches('a.btn.btn-default[title*="Mostrar todos"]')) return scheduleFinalCheck("Mostrar todos");
      if (a.matches('a.btn.btn-default[title*="Intimar todos"]')) return scheduleFinalCheck("Intimar todos");
    }, true);
  }
  ...
  function runAll() {
    ...
    // Copiar ID pode rodar mesmo no boot leve (não “quebra” nada)
  ...
  async function enableAfterLoadAndQuiet() {
    if (!CONFIG.START_AFTER_LOAD) {
      ...
      installFinalCheckClickListener();
      scheduleApply();
      return;
    }
    ...
    if (CONFIG.FINAL_CHECK_FALLBACK_ON_LOAD_SILENT) {
      scheduleFinalCheck("Carregamento inicial", { silent: true });
    }
  }
